...
// const PersonalTrainerQrCheckin = ({ member_id, member_name }) => {
...
//           </div>
//           <div>
//             <h2 className="fw-bold mb-0">Gym Check-in QR Code</h2>
...
const PersonalTrainerQrCheckin = ({ member_name }) => {
  const CODE_TTL = 60; // seconds
  const [qrNonce, setQrNonce] = useState(generateNonce(10));
  const [secondsLeft, setSecondsLeft] = useState(CODE_TTL);
  const [issuedAt, setIssuedAt] = useState(new Date());
  ...
  }, [qrNonce, memberId, member_name, issuedAt]);
...
const fetchCheckinHistory = async () => {
  ...
  // Get status badge based on computedStatus
