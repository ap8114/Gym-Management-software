...
const ReportsAttendance = () => {
    ...
    // NOTE: Showing only records where role === 'Member' (case-insensitive)
    const filteredRecords = records.filter(record => {
        ...
        const role = (record.role || '').toString();
        const status = record.status || '';
        const searchTermLower = searchTerm.toLowerCase();

        // Only include role 'member' or 'housekeeping' (case-insensitive)
        const roleLower = role.toLowerCase();
        if (!(roleLower === 'member' || roleLower === 'housekeeping')) return false;
                                        ...
                                        filteredRecords.map((record) => (
                                            <tr key={record.attendance_id}>
                                                ...
                                                <td className="text-nowrap d-none d-lg-table-cell" style={{ padding: '0.5rem' }}>
                                                    <span className={`badge rounded-pill ${record.mode === 'QR' ? 'bg-info text-white' : 'bg-secondary text-white'
